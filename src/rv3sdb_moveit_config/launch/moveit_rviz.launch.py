...
This file does not start simulation and its own move_group. It assumes mobile_platform_with_arm_launch.py
(with moveit:=true) is already running and has its own move_group node up. This file only builds the same
MoveIt2 config (so RViz's MotionPlanning display has robot_description/SRDF/kinematics/etc. to show) and 
starts RViz2 alone.
...
def generate_launch_description():
    ...
    Starts a plain RViz2 instance parameterized with the MoveIt2 config, so the MotionPlanning display can talk 
    to the move_group that's already running from mobile_platform_with_arm_launch.py.
